...
typedef RangeChangedCallback = void Function(DateTime first, DateTime? last);

class CalendarRangePickerController {
  late void Function(DateTime first, DateTime? last, {bool notify}) setRange;
  late void Function(DateTime dateTime) setStartDate;
  late void Function(DateTime dateTime) setEndDate;

  void init({
    required void Function(DateTime first, DateTime? last, {bool notify}) setRange,
    required void Function(DateTime dateTime) setStartDate,
    required void Function(DateTime dateTime) setEndDate,
  }) {
...
class CalendarDateRangePicker extends StatefulWidget {
  ...
  ///
  /// It will display a grid of days for the [initialDateFrom]'s month. The day
  /// indicated by [initialDateFrom] will be selected.
  ///
  ...
  ///
  /// The [initialDateFrom], [firstDate], [lastDate], [onDateChanged], and
  /// [initialCalendarMode] must be non-null.
  ///
  /// [lastDate] must be after or equal to [firstDate].
  ///
  /// [initialDateFrom] must be between [firstDate] and [lastDate] or equal to
  /// one of them.
  ///
  ...
  CalendarDateRangePicker({
    ...
    Key? key,
  })  : initialDateFrom = DateUtils.dateOnly(initialDateFrom),
    ...
    assert(
      !this.initialDateFrom.isBefore(this.firstDate),
      'initialDate ${this.initialDateFrom} must be on or after firstDate ${this.firstDate}.',
    );
    assert(
      !this.initialDateFrom.isAfter(this.lastDate),
      'initialDate ${this.initialDateFrom} must be on or before lastDate ${this.lastDate}.',
    );
    assert(
      selectableDayPredicate == null || selectableDayPredicate!(this.initialDateFrom),
      'Provided initialDate ${this.initialDateFrom} must satisfy provided selectableDayPredicate.',
    );
  }

  final CalendarRangePickerController controller;
  ...
  final RangeChangedCallback onDateChanged;
...
class _CalendarDateRangePickerState extends State<CalendarDateRangePicker> {
  ...
  void initState() {
    super.initState();
    _selectedDateEnd = widget.initialDateTo;
    _mode = widget.initialCalendarMode;
    _currentDisplayedMonthDate = DateTime(widget.initialDateFrom.year, widget.initialDateFrom.month);
    _selectedDate = widget.initialDateFrom;
    widget.controller.init(setRange: _setRange, setEndDate: _setEndDate, setStartDate: _setStartDate);
  }

  @override
  void didUpdateWidget(CalendarDateRangePicker oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.initialCalendarMode != oldWidget.initialCalendarMode) {
      _mode = widget.initialCalendarMode;
    }
    if (!DateUtils.isSameDay(widget.initialDateFrom, oldWidget.initialDateFrom)) {
      _currentDisplayedMonthDate = DateTime(widget.initialDateFrom.year, widget.initialDateFrom.month);
      _selectedDate = widget.initialDateFrom;
    }
  }
  ...
  void didChangeDependencies() {
    ...
    assert(debugCheckHasMaterial(context), 'MaterialLocalizations.delegate() must be installed.');
    assert(debugCheckHasMaterialLocalizations(context), 'MaterialLocalizations.delegate() must be installed.');
    assert(debugCheckHasDirectionality(context), 'Directionality.delegate() must be installed.');
  ...
  void _setRange(DateTime from, DateTime? to, {bool notify = true}) {
    setState(() {
      _selectedDate = from;
      _selectedDateEnd = to;
    });
    if (notify) {
      widget.onDateChanged(_selectedDate, _selectedDateEnd);
    }
  }
  ...
  void _setEndDate(DateTime dateTime) {
    setState(() {
  ...
  void _handleDayChanged(DateTime value) {
    _vibrate();
    setState(() {
      if (_selectedDateEnd != null || value.isBefore(_selectedDate)) {
        _selectedDate = value;
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  Widget _buildItems(BuildContext context, int index) {
    ...
    return _DayPicker(
      ...
      selectedDateEnd: widget.selectedDateEnd,
      currentDate: widget.currentDate,
...
class _DayPickerState extends State<_DayPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    final TextTheme textTheme = Theme.of(context).textTheme;
    final TextStyle? headerStyle = textTheme.caption?.copyWith(
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        final bool isSelectionStart = DateUtils.isSameDay(widget.selectedDate, dayToBuild);
        final bool isSelectionEnd =
            widget.selectedDateEnd != null && DateUtils.isSameDay(widget.selectedDateEnd, dayToBuild);

        final bool isSelectedDay = isSelectionStart || isSelectionEnd;

        final bool isSelectedBetween = isSelectedDay ||
            (widget.selectedDateEnd != null &&
                dayToBuild.isAfter(widget.selectedDate) &&
        ...
        if (isSelectionStart && isSelectionEnd || (isSelectionStart && widget.selectedDateEnd == null)) {
...
/// [CalendarDateRangePicker], or [showDatePicker] which create full date pickers.
///
/// See also:
///
///  * [CalendarDateRangePicker], which provides a Material Design date picker
///    interface.
///
...
class _YearPickerState extends State<YearPicker> {
  ...
  @override
  Widget build(BuildContext context) {
    assert(debugCheckHasMaterial(context), 'MaterialLocalizations.of(context) is null');
    return Column(
      children: <Widget>[
        const Divider(),
        Expanded(
