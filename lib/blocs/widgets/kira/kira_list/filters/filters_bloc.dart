...
class FiltersBloc<T extends AListItem> extends Bloc<AFiltersEvent, AFiltersState<T>> {
  ...
  void _mapFiltersSearchEventToState(FiltersSearchEvent<T> filtersSearchEvent, Emitter<AFiltersState<T>> emit) {
    List<FilterOption<T>> filterOptions = List<FilterOption<T>>.from(state.activeFilters)..removeWhere((FilterOption<T> e) => e is SearchOption);
  ...
  void _mapFiltersAddOptionEventToState(
    ...
    List<FilterOption<T>> filterOptions = List<FilterOption<T>>.from(state.activeFilters)..add(filtersAddOptionEvent.filterOption);
    _notifyFilterUpdate(filterOptions, emit);
  }
  ...
  void _mapFiltersRemoveOptionEventToState(
    ...
    List<FilterOption<T>> filterOptions = List<FilterOption<T>>.from(state.activeFilters)..remove(filtersRemoveOptionEvent.filterOption);
    _notifyFilterUpdate(filterOptions, emit);
  }
  ...
  void _notifyFilterUpdate(List<FilterOption<T>> filterOptions, Emitter<AFiltersState<T>> emit) {
    ...
    emit(filterOptions.isNotEmpty ? FiltersActiveState<T>(activeFilters: filterOptions, filterComparator: filterComparator) : FiltersEmptyState<T>());
  }
