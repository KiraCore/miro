import 'package:miro/infra/dto/api_kira/query_proposals/a_proposal_type_content.dart';

class ProposalUnassignRoleFromAccount extends AProposalTypeContent {
  final String address;
  final String roleIdentifier;

  const ProposalUnassignRoleFromAccount({
    required String type,
    required this.address,
    required this.roleIdentifier,
  }) : super(type: type);

  factory ProposalUnassignRoleFromAccount.fromJson(Map<String, dynamic> json) {
    return ProposalUnassignRoleFromAccount(
      type: json['@type'] as String,
      address: json['address'] as String,
      roleIdentifier: json['roleIdentifier'] as String,
    );
  }

  @override
  List<Object> get props => <Object>[address, roleIdentifier];
}
