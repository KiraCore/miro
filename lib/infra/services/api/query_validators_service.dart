...
class QueryValidatorsService implements _IQueryValidatorsService {
  ...
  Future<PageData<ValidatorModel>> getValidatorsList(QueryValidatorsReq queryValidatorsReq,
    ...
    try {
      ...
      AppLogger().log(
          message: 'QueryValidatorsService: Cannot parse getValidatorsList() for URI $networkUri: $e',
  ...
  Future<List<ValidatorModel>> getValidatorsByAddresses(List<String> validatorAddresses,
    ...
    QueryValidatorsResp queryValidatorsResp =
        await getQueryValidatorsResp(const QueryValidatorsReq(all: true), forceRequestBool: forceRequestBool);
    return queryValidatorsResp.validators
        .where((Validator e) => validatorAddresses.contains(e.address))
        .map(ValidatorModel.fromDto)
        .toList();
  }
  ...
  Future<Status> getStatus(Uri networkUri, {bool forceRequestBool = false}) async {
    ...
    try {
      ...
      AppLogger().log(
          message: 'QueryValidatorsService: Cannot parse getStatus() for URI $networkUri', logLevel: LogLevel.error);
      throw DioParseException(response: response, error: e);
    }
  }
}
