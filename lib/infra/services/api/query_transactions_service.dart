...
class QueryTransactionsService implements _IQueryTransactionsService {
  ...
  Future<List<TxListItemModel>> getTransactionList(QueryTransactionsReq queryTransactionsReq) async {
    ...
    try {
      ...
      List<TxListItemModel> txListItemModels = List<TxListItemModel>.empty(growable: true);
      for( Transaction transaction in queryTransactionsResp.transactions) {
        TxListItemModel txListItemModel = await TxListItemModel.buildFromDto(transaction);
        txListItemModels.add(txListItemModel);
      }
      return txListItemModels;
    } catch (e) {
