...
Future<void> main() async {
  ...
  group('Test of QueryGovernanceProposalService.getGovernanceProposal() method', () {
    test('Should return [Proposal] for a specific proposal', () async {
      TestUtils.printInfo('Data request');
      try {
        Proposal actualProposal = await actualQueryGovernanceProposalService.getGovernanceProposal(proposalId: 25);

        TestUtils.printInfo('Data return');
        print(actualProposal);
        print('');
      } on DioConnectException catch (e) {
        TestUtils.printError('query_governance_proposal_test.dart: Cannot fetch [GovernanceProposal] for URI $networkUri: ${e.dioError.message}');
      } on DioParseException catch (e) {
